...
class TestExceptions:
    ...
    def test_json_read_error(   self) -> None:
        ...
        with pytest.raises(FileReadError)    as exc_info:
            raise          FileReadError("Test error message")
...
class Testreadfromfile:
    ...
    def test_readfromfile_valid_file(           self) -> None:
        ...
        try:
            result = readfromfile(temp_filename)
            assert result == test_data
        finally:
            import os
            os.unlink(temp_filename)

    def test_readfromfile_invalid_json_content( self) -> None:
        ...
        try:
            with pytest.raises(FileReadError, match="Invalid JSON File"):
                readfromfile(temp_filename)
        finally:
            import os
            os.unlink(temp_filename)

    def test_readfromfile_file_not_found(       self) -> None:
        """Test reading a non-existent file"""
        with pytest.raises(FileReadError, match="Invalid JSON File"):
            readfromfile("non_existent_file.json")

    @patch('builtins.open')
    def test_readfromfile_permission_error(     self, mock_open: Mock) -> None:
        """Test reading a file with permission issues"""
        # Mock open to raise PermissionError
        mock_open.side_effect = PermissionError("Permission denied")

        with pytest.raises(FileReadError, match="Invalid JSON File"):
            readfromfile("some_file.json")

    @patch('builtins.open')
    def test_readfromfile_os_error(             self, mock_open: Mock) -> None:
